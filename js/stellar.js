 ...
 * @return
 */
(function(d, f, g, b) { if (!d) { console.error("jQuery nÃ£o encontrado, seu plugin jQuery nÃ£o irÃ¡ funcionar."); return false }(function() { var k = 0,
            l = ["ms", "moz", "webkit", "o"]; for (var j = 0; j < l.length && !f.requestAnimationFrame; ++j) { f.requestAnimationFrame = f[l[j] + "RequestAnimationFrame"];
            f.cancelAnimationFrame = f[l[j] + "CancelAnimationFrame"] || f[l[j] + "CancelRequestAnimationFrame"] } if (!f.requestAnimationFrame) { f.requestAnimationFrame = function(q, n) { var m = new Date().getTime(); var o = Math.max(0, 16 - (m - k)); var p = f.setTimeout(function() { q(m + o) }, o);
                k = m + o; return p } } if (!f.cancelAnimationFrame) { f.cancelAnimationFrame = function(m) { clearTimeout(m) } } })(); var e = "parallax",
        c = { on: "scroll", listenTo: f, sceneMode: false },
        a = d(f),
        i = 0;

    function h(k, j) { this._name = e;
        this._instance_id = ++i;
        this.el = k;
        this.$el = d(k);
        ...
        this.init() }
    d.extend(h.prototype, { init: function() { var j = this;
            this.$triggerOrigin.on(j.settings.on + "." + j._name, function() { j.parallaxTranslate() });
            j.parallaxTranslate() }, parallaxTranslate: function() { var j = this; if (j.inScreen()) { f.requestAnimationFrame(function() { var k = a.scrollTop() - j.$el.offset().top;
                    j.$el.css("transform", "translateY(" + k / 2 + "px)") }) }
            console.groupEnd() }, destroy: function() { this.$el.removeData();
            d(this.settings.listenTo).off("." + e) }, somePublicMethod: function(k, j) { privateMethod.call(this) }, inScreen: function(l) { var o; if (typeof l !== "boolean" && l !== b) { o = d(l);
                l = arguments[1] || false } else { o = this.$el;
                l = l || false } var k = a.scrollTop(),
                n = k + a.height(),
                j = o.offset().top,
                m = j + o.height(); if (l === true) { return k <= j && n >= m } return !(k > m || n < j) } });
